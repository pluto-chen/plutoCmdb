...
class AssetAdmin(admin.ModelAdmin):
    list_display = ('id','asset_type','project','name','manufactory','idc','admin')
    inlines = [ServerInline,CPUInline,RAMInline,DiskInline]
    search_fields = ['name',]
    list_filter = ['idc','manufactory','asset_type']
    choice_fields = ('asset_type',)
    fk_fields = ('manufactory','idc','admin')
    list_per_page = 10
    #list_filter = ('asset_type','status','manufactory','idc','business_unit','admin','trade_date')
    dynamic_fk = 'asset_type'
    dynamic_list_display = ('server_type','os_type','os_release')
    dynamic_choice_fields = ('server_type',)
    #m2m_fields = ('tags',)
'''
...
class EventLogAdmin(admin.ModelAdmin):
    list_display = ('name','asset','detail','date','user')
    search_fields = ('asset',)
